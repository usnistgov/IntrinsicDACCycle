...
def Build_GCMC_json(temperature, target_p, Sorbent, Sorbates, y):
    ...
    args_GCMC = {
        ...
        # 'trials_EQ': int(1.e5), #Equilibration Burn-in trials
        # 'trials_PROD': int(5.e5), #Production Run (data collected)       
        'box': box,
        'tilts': tilts,
...
def run_gcmc(GCMC_args):
    ...
    mc.set(fst.MakeRandomMT19937(fst.args({'seed': str(GCMC_args['seed'])})))
    ...
    mc.add(
        fst.MakeCheckEnergy(
            fst.args({
                'trials_per_update': str(steps_per),
                'tolerance': '0.0001'
            })))
    mc.add(
        fst.MakeTune(
            fst.args({
                'trials_per_tune': str(steps_per),
                #"stop_after_phase": "0"
            })))
    ...
    mc.add(
        fst.MakeEnergy(
            fst.args({
                'output_file': 'en' + '.txt',
                'trials_per_write': str(steps_per),
                'trials_per_update': '1'
            })))
    #  Particle Counter
    ...
    E_average = energy_analyzer.accumulator().average()
    E_std = energy_analyzer.accumulator().stdev()
    E_bstd = energy_analyzer.accumulator().block_stdev()
    ...
    # subtract 1 because the the "last" particle is the MOF
        ...
        num_average = num_analyzer.accumulator().average() #average number of particles
        num_std = num_analyzer.accumulator().stdev()
        num_bstd = num_analyzer.accumulator().block_stdev()
